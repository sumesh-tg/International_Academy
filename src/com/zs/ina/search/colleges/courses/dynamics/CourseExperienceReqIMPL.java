 ...
 * @author 100035
 */
public class CourseExperienceReqIMPL implements CourseExperienceReqDAO {
    ...
    public boolean insertExperience(ExperienceReqBEAN experienceReqBEAN) {
        ...
        String sql = "INSERT INTO " + TableNames.TBL_SR_EDU_COURSE_EXP_REQ + " (\n"
                + "	exp_reqmt_id,\n"
                + "	course_id,\n"
                + "	profession,\n"
                + "	experince_duration\n"
                ...
                + "VALUES\n"
                + "	(?, ?, ?, ?);";
        int row = jdbcTemplate.update(sql, new Object[]{experienceReqBEAN.getRowId(), experienceReqBEAN.getCourseId(),
    ...
    @Override
    public boolean deleteExperience(String expReqmtId) {
        jdbcTemplate = new JdbcTemplate(dataSource);
        String sql = "DELETE FROM " + TableNames.TBL_SR_EDU_COURSE_EXP_REQ + "\n"
                + " WHERE\n"
                + "exp_reqmt_id=?";
    ...
    public boolean updateExperience(ExperienceReqBEAN experienceReqBEAN) {
        jdbcTemplate = new JdbcTemplate(dataSource);
        String sql = "UPDATE " + TableNames.TBL_SR_EDU_COURSE_EXP_REQ + "\n"
                + "SET profession = ?,\n"
                + " experince_duration = ?\n"
                + "WHERE\n"
                + "	exp_reqmt_id = ?";
    ...
    @Override
    public ObservableList<ExperienceReqBEAN> retrieveExperience(String courseId) {
     ObservableList<ExperienceReqBEAN> listExperience = FXCollections.observableArrayList();
        jdbcTemplate = new JdbcTemplate(dataSource);
        String query = "SELECT * FROM " + TableNames.TBL_SR_EDU_COURSE_EXP_REQ + " WHERE course_id = ?";
        List<ExperienceReqBEAN> temp = jdbcTemplate.query(query, new Object[]{courseId}, new ExperienceMapper());
    ...
    public class ExperienceMapper implements RowMapper<ExperienceReqBEAN> {
        ...
        public ExperienceReqBEAN mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            experienceReqBEAN.setCourseId(rs.getString("course_id"));
