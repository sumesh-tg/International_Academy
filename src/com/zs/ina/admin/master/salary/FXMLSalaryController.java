 ...
 */
package com.zs.ina.admin.master.salary;

import com.zs.ina.admin.master.excelupload.ExcelUpload;
import com.zs.ina.admin.master.salary.dao.SalaryBean;
import com.zs.ina.admin.master.salary.dao.SalaryDAO;
import java.net.URL;
...
public class FXMLSalaryController implements Initializable {
    ...
    private void handleSaveBtn(ActionEvent event) {
        ...
        if (validation()) {
            if (!languageId.equals("")) {
                int row = SalaryDAO.updateSalary(languageBEAN);
                if (row > 0) {
                    setTableValues();
                    clear();
                }
                ...
                if (!SalaryDAO.checkDuplicateEntry(salaryTxt.getText())) {
                    int row = SalaryDAO.insertSalary(languageBEAN);
                    if (row > 0) {
                        setTableValues();
                        clear();
                    }
                    ...
                    usernameValidator.getItems().add(
                            new MenuItem("Salary already inserted! "));
    ...
    private void autoCompletion() {
        List<String> allLaguages = SalaryDAO.getAllSalries();
    ...
    private void setTableValues() {
        ObservableList<SalaryBean> languageBEANs = SalaryDAO.getSalaryDetails();
